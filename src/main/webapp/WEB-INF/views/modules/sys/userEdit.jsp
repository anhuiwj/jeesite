...
<body>
	<div class="mini-panel" title="用户信息" style="width:100%;" showCollapseButton="false">
		<div id="uForm">
			<input id="id" class="mini-hidden" name="id" value="${user.id }">
			...
			<fieldset style="border:solid 1px #aaa;padding:3px;">
				...
				<table>
					<tr>
						...
						<td>
							<input id="loginName" name="loginName" value="${user.loginName }" class="mini-textbox" enabled="false"/>
						</td>
						<td style="width:30%;">工号：</td>
						<td>
							<input id="no" name="no" class="mini-textbox" required="true" value="${user.no }"/>
						</td>
					</tr>
					<tr>
						<td>密码：</td>
						<td>
							<input id="password" name="password" class="mini-password"  onKeyUp="checkIntensity"/>
						</td>
						<td></td>
						<td>
					...
					<tr>
						<td>重复密码：</td>
						<td>
							<input id="password2" name="password2" class="mini-password" />
						</td>
						<td></td>
						<td></td>
					</tr>
					...
					<tr>
						<td>姓名：</td>
						<td>
							<input id="name" name="name" class="mini-textbox" value="${user.name }" required="true"/>
						</td>
						<td>机构：</td>
						<td>
							<!--modify xf 2016,12.8 -->
							...
							<c:if test="${fns:getUser().admin}"><!-- 如果是超级管理员-->
								<div id="orgCode" name="orgCode" class="mini-treeselect" value="${user.realOrgCode }"  required="true" resultAsTree="false" style="width:260px;" data="${fns:getOfficeTreeForAdmin('0')}" ></div>
							</c:if>
							<c:if test="${! fns:getUser().admin}"><!-- 如果不是超级管理员-->
								<c:if test="${manager == '1'}">	<!--系统管理员-->
									<div id="orgCode" name="orgCode" class="mini-treeselect" value="${user.realOrgCode}" resultAsTree="false" style="width:260px;" data="${fns:getOfficeTreeForArea("310000000000")}" ></div>
								</c:if>
								<c:if test="${manager == '2'}">	<!--区县管理员-->
									<div id="orgCode" name="orgCode" class="mini-treeselect" value="${user.realOrgCode}" resultAsTree="false" style="width:260px;" data="${fns:getOfficeTreeForArea(areaCode)}" ></div>
								</c:if>
								<c:if test="${manager == '3'}">	<!--机构管理员-->
									<div id="orgCode" name="orgCode" class="mini-treeselect" value="${user.realOrgCode}" resultAsTree="false" style="width:260px;" data="${fns:getOfficeTreeForArea(orgCode)}" ></div>
								</c:if>
							</c:if>
					...
					<tr>
						<td>手机：</td>
						<td>
							<input id="mobile" name="mobile" value="${user.mobile }" class="mini-textbox"/>
						</td>
						<td>邮箱：</td>
						<td>
							<input id="email" name="email" value="${user.email }" class="mini-textbox"/>
						</td>
					</tr>
				</table>
			</fieldset>
		...
		<div class="mini-panel" title="角色" style="width:100%;height:60%;overflow:auto">
			...
			<div id="roles" name="roles" class="mini-checkboxlist" repeatItems="4" style="width:100%;height:100%;overflow:auto"
				 repeatLayout="table" r textField="text" valueField="value"  value="${user.rolesValue }"
	...
	<%--    <div showCollapseButton="false">
           <div class="mini-panel" title="资源权限" style="width:100%;height:100%;overflow:auto">
                   ...
                   <ul id="resTree" class="mini-tree" url="${ctx}/sys/user/menuTree?userId=${user.id }" checkRecursive="true"
